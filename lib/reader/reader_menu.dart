...
class _ReaderMenuState extends State<ReaderMenu> with SingleTickerProviderStateMixin {
  ...
  buildTopView(BuildContext context) {
    return Positioned(
      ...
      right: 0,
      child: Container(
        decoration: BoxDecoration(color: TYColor.paper, boxShadow: Styles.borderShadow),
    ...
    return Container(
      ...
      child: Row(
        ...
        children: <Widget>[
          ...
          Text('${percentage.toStringAsFixed(1)}%', style: TextStyle(color: TYColor.lightGray, fontSize: 12)),
        ],
      ),
    );
  }
    ...
    return Container(
      ...
      child: Column(
        children: <Widget>[
          Image.asset(icon),
          SizedBox(height: 5),
          Text(title, style: TextStyle(fontSize: fixedFontSize(12), color: TYColor.darkGray)),
        ],
      ),
    );
  }
